...
describe("op - generic operations", () => {
  it("creates a stored terminal result that reacts to all initially evaluated operands", () => {
    const input = signal(1);
    const comparison = signal(1);
    const whenTruthy = signal("yes");
    const whenFalsy = signal("no");
    const label = op(input).equals(comparison).then(whenTruthy, whenFalsy);

    expect(label.value).toBe("yes");

    comparison.value = 2;
    expect(label.value).toBe("no");

    input.value = 2;
    expect(label.value).toBe("yes");

    whenTruthy.value = "confirmed";
    expect(label.value).toBe("confirmed");

    whenFalsy.value = "rejected";
    expect(label.value).toBe("confirmed");
  });

  it("keeps the fixed dependencies selected by short-circuiting on its first terminal evaluation", () => {
    const input = signal(true);
    const fallback = signal("fallback");
    const result = op(input).or(fallback).result;

    expect(result.value).toBe(true);

    fallback.value = "updated fallback";
    expect(result.value).toBe(true);

    input.value = false;
    expect(result.value).toBe("updated fallback");
  });
...
describe("op - array operations", () => {
  ...
  it("supports readonly arrays", () => {
    const value = signal<readonly number[]>([1, 2, 3]);
    const hasThreeItems = op(value).lengthEquals(3).truthy;

    expect(hasThreeItems.value).toBe(true);

    value.value = [1];
    expect(hasThreeItems.value).toBe(false);
  });
});
